...
# Remote Git Cast Tasks (`cast.task`)

## Overview

Currently, remote tasks resolve to a Git repository and execute a root `.ts` or `.js` file directly. To enable more powerful, declarative, and reusable tasks, Cast should support a remote task execution model driven by a `cast.task` definition file.
...
### 3. Task Definition Schema (`cast.task`)
- Look for a `cast.task` (or `.yml`) file in the targeted directory (root or subfolder).
...
- **Definition:** The `cast.task` should define an `inputs:` block specifying expected parameters (with optional types or defaults).
- **Validation:** When `cast` runs the task, it must evaluate the `with:` arguments from the caller's `castfile.yaml` at runtime, ensuring all required inputs are met and validating them against the `cast.task` definition.
...
**Remote Definition (`cast-tasks/linter/cast.task`):**
```yaml
name: "Linter Task"
