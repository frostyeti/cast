...
func (he *HostElement) UnmarshalYAML(value *yaml.Node) error {
	if he == nil {
		he = &HostElement{}
	}
	...
	if value.Kind == yaml.ScalarNode {
		...
		if strings.ContainsRune(hostStr, ':') {
			...
			if err != nil {
				return errors.YamlErrorf(value, "invalid port number: %v", err)
			}
			if portV < 0 || portV > 65535 {
				return errors.YamlErrorf(value, "port number out of range: %d", portV)
			}
